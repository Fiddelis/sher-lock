...
void setupCamera()
{
  ...
  config.xclk_freq_hz = 20000000; // Aumentar a frequência do clock para 20 MHz
  config.pixel_format = PIXFORMAT_GRAYSCALE;
  config.frame_size = FRAMESIZE_QVGA;
  config.jpeg_quality = 15;
  config.fb_count = 2; // Aumentar o tamanho do buffer de quadros
  ...
  Serial.println("Câmera configurada e inicializada com sucesso.");
  Serial.println();
}
...
void QRCodeReader(void *pvParameters)
{
  ...
  Serial.print("QRCodeReader rodando no núcleo ");
  ...
  {
    processQRCode();
    vTaskDelay(10 / portTICK_PERIOD_MS); // Adicionar atraso para controlar a taxa de captura
  }
}
